...
def main():
    ...
    for PELE_sim_path in all_sim_it:
        ...
        for col in data.columns:
            if ('_net_charge' in col):
                if (col not in columns):
                    columns.append(col)

    print('   - Subpockets found:')
    for col in columns:
        print('     - {}'.format(col.strip('_net_charge')))

    if (len(columns) == 0):
        raise ValueError('Subpocket net charges were missing in '
                         + 'the simulation paths that were supplied')

    fig, axs = plt.subplots(len(columns), 1, figsize=(20, 15))
    fig.suptitle('LIG net charges within subpocket')

    for i, col in enumerate(columns):
        axs[i].set_title(col.strip('_net_charge'))
        axs[i].set_ylabel('LIG net charges within subpocket '
                          + '{}'.format(col.strip('_net_charge'))
                          + '(Atomic units)')
    ...
    for PELE_sim_path in all_sim_it:
        ...
        print('   - Retrieving subpocket net charges')
        data = pd.read_csv(PELE_sim_path.joinpath(csv_file_name))
    ...
    fig, axs = plt.subplots(int(len(columns) / 2) + len(columns) % 2, 2,
                            figsize=(15, 5 * int(len(columns) / 2
                                                 + len(columns) % 2)))
    fig.suptitle('Subpocket net charges vs -pIC50')
    ...
    for i, col in enumerate(columns):
        ax = axs[int(i / 2)][i % 2]
        ax.set_title(col.strip('_net_charge'))
        ax.set_ylabel('{}-percentile of {} net charges'.format(
            percentile, col.strip('_net_charge')))
        ax.set_xlabel('-pIC50')
    ...
    plt.savefig('subpocket_netcharge_correlations.png')
    plt.close()
