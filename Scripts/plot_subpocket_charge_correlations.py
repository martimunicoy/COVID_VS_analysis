...
def parse_args():
    ...
    parser.add_argument('-t', '--charge_type', default='net',
                        nargs='?', type=str,
                        choices=['net', 'positive', 'negative'],
                        help='Type of charge to be plotted: net, positive or '
                        + 'negative. Default: net')

    parser.add_argument('-o', '--output', metavar='STR', type=str,
                        default='subpocket_charge_correlations.png',
                        help='Output name for the resulting plot')

    args = parser.parse_args()

    return args.traj_paths, args.subpockets, args.ic50, args.percentile, \
        args.charge_type, args.output


def main():
    # Parse args
    PELE_sim_paths, csv_file_name, ic50_csv, percentile, charge_type, \
        output = parse_args()
    ...
    for PELE_sim_path in all_sim_it:
        ...
        for col in data.columns:
            if ('_{}_charge'.format(charge_type) in col):
                if (col not in columns):
                    columns.append(col)

    print('   - Subpockets found:')
    for col in columns:
        print('     - {}'.format(col.strip('_{}_charge'.format(charge_type))))

    if (len(columns) == 0):
        raise ValueError('Subpocket {} '.format(charge_type)
                         + 'charges were missing in the simulation paths'
                         + ' that were supplied')

    fig, axs = plt.subplots(len(columns), 1, figsize=(20, 15))
    fig.suptitle('LIG {} charges within subpocket'.format(charge_type))

    for i, col in enumerate(columns):
        axs[i].set_title(col.strip('_{}_charge'.format(charge_type)))
        axs[i].set_ylabel(
            'LIG {} charges within '.format(charge_type)
            + 'subpocket {}'.format(
                col.strip('_{}_charge'.format(charge_type)))
            + '(Atomic units)')
    ...
    for PELE_sim_path in all_sim_it:
        ...
        print('   - Retrieving subpocket {} charges'.format(charge_type))
        data = pd.read_csv(PELE_sim_path.joinpath(csv_file_name))
    ...
    fig, axs = plt.subplots(int(len(columns) / 2) + len(columns) % 2, 2,
                            figsize=(15, 5 * int(len(columns) / 2
                                                 + len(columns) % 2)))
    fig.suptitle('Subpocket {} charges vs -pIC50'.format(charge_type))
    ...
    for i, col in enumerate(columns):
        ax = axs[int(i / 2)][i % 2]
        ax.set_title(col.strip('_{}_charge'.format(charge_type)))
        ax.set_ylabel('{}-percentile of {} {} charges'.format(
            percentile, col.strip('_{}_charge'.format(charge_type)),
            charge_type))
        ax.set_xlabel('-pIC50')
